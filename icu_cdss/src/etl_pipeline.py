...
from config import MIMIC_HOSP_DIR, MIMIC_ICU_DIR, PROCESSED_DIR, RESAMPLE_FREQ, VITAL_ITEMIDS, LAB_ITEMIDS
...
def run_etl(max_rows: int | None = None) -> pd.DataFrame:
    ...
    keep_cols = ["stay_id", "hadm_id", "subject_id", "chart_hour"] + list(VITAL_ITEMIDS.keys()) + list(LAB_ITEMIDS.keys())
